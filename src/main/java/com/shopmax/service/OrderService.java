...
public class OrderService {
	...
	public Long order(OrderDto orderDto, String email) {
		...
		// 2.현재 로그인한 회원의 이메일을 이용해 회원정보 조회
	...
	public Page<OrderHistDto> getOrderList(String email, Pageable pageable) {
		...
		// 1.유저 아이디와 페이징조건을 이용하여 주문 목록을 조회
		...
		// 3.주문 리스트를 순회하면서 구매 이력 페이지에 전달할 Dto(OrderHistDto)를 생성한다.
	...
	public boolean validateOrder(Long orderId, String email) {
		Member curMember = memberRepository.findByEmail(email);	// 로그인한 사용자를 찾는다.
		// orderId로 주문내역 찾기
		Order order = orderRepository.findById(orderId)
									 .orElseThrow(EntityNotFoundException::new);
		
		Member savedMember = order.getMember();	// 주문한 사용자찾기
	...
	// 주문 삭제
	// delete하기 전에 select를 한 번 해준다.
	// -> 영속성 컨텍스트에 엔티티를 저장한 후 변경감지를 하기위해 사용
