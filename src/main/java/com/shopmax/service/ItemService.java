...
public class ItemService {
	...
	// 등록한 상품데이터 가져오기
	@Transactional(readOnly = true)	// 트랜잭션 읽기 전용(변경감지를 수행하지않음) -> 성능향상
	public ItemFormDto getItemDtl(Long itemId) {
		// 1.item_img 테이블의 이미지를 가져온다.
		List<ItemImg> itemImgList = itemImgRepository.findByItemIdOrderByIdAsc(itemId);
		
		// ItemImg 엔티티 -> ItemImgDto로 변환 (view단에서 사용하는 것은 dto이고 컨트롤러에게 보낼때는 dto를 이용해야하기 때문)
		...
		// Item entity 객체 -> dto로 변환
		ItemFormDto itemFormDto = ItemFormDto.of(item);
		
		// 3.ItemFormDto에 이미지정보 (itemImgDtiList)를 넣어준다.
