...
public class ItemController {
	...
	public String itemShopList(Model model, ItemSearchDto itemSearchDto, Optional<Integer> page) {
		...
		Pageable pageable  = PageRequest.of(page.isPresent() ? page.get() : 0, 6);	// of(조회할 페이지의 번호: 0부터 시작, 한 페이지 당 조회할 데이터의 갯수)
	...
	public String itemNew(@Valid ItemFormDto itemFormDto, BindingResult bindingResult, Model model, @RequestParam("itemImgFile") List<MultipartFile> itemImgFileList) {
		...
		// 상품등록전 첫번째 이미지가 있는지 없는지 검사(첫번째 이미지는 필수 입력값)
	...
	public String itemDtl(@PathVariable("itemId") Long itemId, Model model) {
		...
		try {
			...
			e.printStackTrace();
			model.addAttribute("errorMessage", "상품 정보를 가져올 때 에러가 발생했습니다.");
